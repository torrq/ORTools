...
public sealed class WorkerService : IDisposable
{
    ...
    public void Send<T>(T command) where T : IIpcMessage => _ = SendAsync(command);
    ...
    private async Task RunSessionAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            attempts++;
            if (attempts == 2 && !WorkerLauncher.IsRunning()) WorkerLauncher.TryLaunch();
            await Task.Delay(attempts < 5 ? 1000 : 3000, ct);
        }
        if (!ct.IsCancellationRequested) await ReadLoopAsync(ct);
    }
    ...
    private async Task<bool> TryConnectAsync(CancellationToken ct)
    {
        ...
        {
            _pipe?.Dispose(); _pipe = null; _writer = null;
            return false;
        }
    }
    ...
    private void Dispatch(IpcEnvelope env)
    {
        switch (env.Type)
        {
            case MessageTypes.WorkerReady:
                ...
                break;
            case MessageTypes.AppState: AppStateReceived?.Invoke(env.As<AppStateUpdate>()!); break;
            case MessageTypes.ClientState: ClientStateReceived?.Invoke(env.As<ClientStateUpdate>()!); break;
            case MessageTypes.HpSp: HpSpReceived?.Invoke(env.As<HpSpUpdate>()!); break;
            case MessageTypes.Character: CharacterReceived?.Invoke(env.As<CharacterUpdate>()!); break;
            case MessageTypes.ProcessList: ProcessListReceived?.Invoke(env.As<ProcessListUpdate>()!); break;
            case MessageTypes.ProfileList: ProfileListReceived?.Invoke(env.As<ProfileListUpdate>()!); break;
            case MessageTypes.LogMessage: LogMessageReceived?.Invoke(env.As<LogMessageUpdate>()!); break;
            case MessageTypes.Error: ErrorReceived?.Invoke(env.As<ErrorUpdate>()!); break;
            case MessageTypes.AutopotHPConfig:
                AutopotHPConfigReceived?.Invoke(env.As<AutopotHPConfigUpdate>()!); break;
            case MessageTypes.AutopotSPConfig:
                AutopotSPConfigReceived?.Invoke(env.As<AutopotSPConfigUpdate>()!); break;
            default:
