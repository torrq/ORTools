...
public partial class SkillTimerView : UserControl
{
    public SkillTimerView()
    {
        InitializeComponent();
    }
    ...
    {
        if (sender is TextBox tb && tb.DataContext is SkillTimerSlotViewModel slot)
        {
    ...
    private void DelayTextBox_PreviewKeyDown(object sender, KeyEventArgs e)
    {
        if (sender is TextBox tb && tb.DataContext is SkillTimerSlotViewModel slot)
        {
            ORTools.UI.Helpers.InputHelper.HandleNumericUpDown(tb, e, 10, 600000, 100, newVal => slot.Delay = newVal);
        }
    }
