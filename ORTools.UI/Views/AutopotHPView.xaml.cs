...
public partial class AutopotHPView : UserControl
{
    ...
    private void Row_PreviewMouseMove(object sender, MouseEventArgs e)
    {
        ...
        if (sender is not Border border || border.DataContext is not AutopotHPSlotViewModel slot)
            return;
    ...
    {
        if (sender is not Border targetBorder || targetBorder.DataContext is not AutopotHPSlotViewModel targetSlot)
            return;

        if (!e.Data.GetDataPresent(typeof(int)) || DataContext is not AutopotHPViewModel viewModel)
            return;
    ...
    private static void MoveSlot(ObservableCollection<AutopotHPSlotViewModel> slots, int sourceId, int targetId)
    {
    ...
    {
        if (sender is not TextBox textBox || textBox.DataContext is not AutopotHPSlotViewModel slot)
            return;
    ...
    private void NumericTextBox_TextChanged(object sender, TextChangedEventArgs e)
    {
        ...
        if (textBox.DataContext is AutopotHPSlotViewModel slot)
            slot.Percent = value;
        else if (ReferenceEquals(textBox, DelayBox) && DataContext is AutopotHPViewModel viewModel)
            viewModel.Delay = value;
    }

    private void DelayBox_LostFocus(object sender, RoutedEventArgs e)
    {
        if (sender is TextBox textBox && DataContext is AutopotHPViewModel viewModel
    ...
    {
        if (sender is TextBox textBox && textBox.DataContext is AutopotHPSlotViewModel slot
            && !int.TryParse(textBox.Text, out _))
        {
